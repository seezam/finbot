...
async function getMenuText() {
  ...
  // Форматируем баланс - скрываем весь текст целиком
  const balanceEmoji = total >= 0 ? '💰' : '📉';
  // Экранируем двоеточие и пробел для MarkdownV2
  const balanceLabel = 'Баланс:'.replace(/[.*_\[\]()~`>#+\-=|{}.!]/g, '\\$&');
  const balanceText = `*||${balanceEmoji} ${balanceLabel} ${totalStr}||*`;
  return `💼 Меню ${emoji}\n\n${balanceText}`;
}
