...
async function getMenuText() {
  ...
  // Форматируем баланс с эмодзи и отступами для лучшего отображения
  // Используем неразрывные пробелы и эмодзи для визуального разделения
  const balanceEmoji = total >= 0 ? '💰' : '📉';
  const balanceText = `${balanceEmoji} Баланс: *||${totalStr}||*`;
  return `💼 Меню ${emoji}\n\n${balanceText}`;
}
